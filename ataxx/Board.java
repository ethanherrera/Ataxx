...
class Board {
    ...
    static char[] linToIndex(int sq, int toSq) {
        char[] normIndex = new char[4];
        normIndex[0] = (char) ((sq - (EXTENDED_SIDE * 2 + 2))
                % EXTENDED_SIDE + 'a');
        normIndex[1] = (char) ((sq - (EXTENDED_SIDE * 2 + 2))
                / EXTENDED_SIDE + '1');
        normIndex[2] = (char) ((toSq - (EXTENDED_SIDE * 2 + 2))
                % EXTENDED_SIDE + 'a');
        normIndex[3] = (char) ((toSq - (EXTENDED_SIDE * 2 + 2))
                / EXTENDED_SIDE + '1');
        return normIndex;
    }
    ...
    boolean legalMove(Move move) {
        ...
        int colDiff = Math.abs(c1 - c0);
        int rowDiff = Math.abs(r1 - r0);

        return get(c0, r0) == whoseMove() && get(c1, r1)
                == EMPTY && colDiff <= 2 && rowDiff <= 2;
    }
    ...
    void makeMove(Move move) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                if (get(neighbor) == whoseMove().opposite()) {
                    addUndo(neighbor);
                    set(neighbor, whoseMove());
                    incrPieces(whoseMove(), 1);
    ...
    void undo() {
        while (_undoSquares.peek() != null) {
            int undoneSquare = _undoSquares.pop();
            PieceColor undonePiece = _undoPieces.pop();
            incrPieces(get(undoneSquare), -1);
            incrPieces(undonePiece, 1);
            unrecordedSet(undoneSquare, undonePiece);

        }
        _undoSquares.pop();
        if (_isJump.peek() != null && _isJump.pop()) {
            _numJumps--;
        }
        _whoseMove = _whoseMove.opposite();
    ...
    boolean legalBlock(char c, char r) {
        char oppositeCol = (char) ('g' - c + 'a');
        char oppositeRow = (char) ('7' - r + '1');
        return allMoves().size() == 0 && get(c, r) == EMPTY
                && get(oppositeCol, oppositeRow) == EMPTY
                && get(oppositeCol, r) == EMPTY
                && get(c, oppositeRow) == EMPTY;
    }

    /** Return true iff it is legal to place a block at CR. */
    boolean legalBlock(String cr) {
        return cr.length() == 2 && legalBlock(cr.charAt(0), cr.charAt(1));
    }
    ...
    void setBlock(char c, char r) {
        ...
        char oppositeRow = (char) ('7' - r + '1');
        set(c, r, BLOCKED);
        set(oppositeCol, r, BLOCKED);
        set(c, oppositeRow, BLOCKED);
        set(oppositeCol, oppositeRow, BLOCKED);
     ...
     *  game). */
    List<Move> allMoves() {
        return Objects.requireNonNullElseGet(_allMoves, ArrayList::new);
    ...
    /** Stack indicating whether the last move was a jump or not. */
    private Stack<Boolean> _isJump;
